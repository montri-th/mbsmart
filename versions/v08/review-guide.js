...
fetch('./assets/smart-review-matrix.json').then(r=>{if(!r.ok)throw Error('matrix');return r.json();}).then(data=>{
 for(const [key,label] of Object.entries(areas)){
  const details=document.createElement('details');details.className='requirement-group';const summary=document.createElement('summary');summary.textContent=label+' · '+data.rows.filter(r=>r.area===key).length+' รายการ';details.append(summary);
  for(const row of data.rows.filter(r=>r.area===key)){
   ...
   const meta=document.createElement('p');meta.className='requirement-meta';meta.textContent=(statuses[row.standard]||row.standard)+' · PDF p'+row.sourcePages.join(', ');article.append(meta);
