...
(() => {
 ...
 const mercedes=window.BC_MERCEDES(T,{mesh,box,tube,sign,M});
 ...
 function car(c,index,parent){return c.brand==='smart'?smart.car(c,parent):mercedes.car(c,parent);}
 ...
 guide.computeLineDistances();guide.name=id;guide.userData={kind:'operating-sequence',sweptPathVerified:false,prerequisite:'Temporarily move MB5 and its price stand; convert mechanical steps to ramp; clear people/cables; return to steps after transfer'};handoverGuide.add(guide);
 }
 floorLabel('1 · MOVE MB5 FIRST',28.2,8.8,3.3,handoverGuide);
 floorLabel('2 · MB6 VIA MB5',32.1,5.25,3.2,handoverGuide);
 const rampNote=floorLabel('3 · CONVERT STEPS TO RAMP',28.2,-1,4,handoverGuide);rampNote.position.y=.07;
 ...
 presets.plan.title='ผังปรับปรุง · Small / Sub Stage 2 จอ / ลำดับส่งมอบ';presets.service.p=[30.5,2.1,-9.8];
 presets.site={p:[68,44,44],t:[22,4,-17],ceiling:true,site:true,upper:true,title:'อาคารและ Workshop · ป้าย MB ตรงระนาบด้านเสม็ด–อ่างศิลา · ที่จอดตามเจ้าของ'};
 presets.frontage={p:[24,18,39],t:[21,7,-3],ceiling:true,site:true,upper:true,title:'มุมยกระดับด้านหน้า · อาคารเดิมและลานด้านหน้า'};
 presets.siteplan={p:[22,86,-17],t:[22,-.6,-17.1],ceiling:false,site:true,upper:false,title:'ผังบริเวณ · ห้องประชุมแยก Sales / Workshop ด้านหลัง / เส้นสีไม่ใช่แนวเขต'};
 presets.workshop={p:[49,29,-6],t:[23,0,-30],ceiling:false,site:true,upper:false,title:'Workshop · ผังเตรียม smart HV + M/E · อุปกรณ์เป็นข้อเสนอรออนุมัติ'};
 presets['workshop-detail']={p:[30,4.1,-29.4],t:[36,1.2,-39.1],ceiling:true,site:true,upper:false,title:'smart Workshop · ช่อง HV และ M/E · อุปกรณ์และระยะรอตรวจ'};
 presets['workshop-plan']={p:[24,48,-29],t:[24,-.6,-29.1],ceiling:false,site:true,upper:false,title:'ผัง Workshop · smart HV + M/E และส่วนใช้ร่วม · ขอบเขตโมเดลรอตรวจ'};
 ...
 function zoom(f){if(cameraLocked)return;const v=camera.position.clone().sub(target);v.multiplyScalar(f);v.clampLength(.6,290);camera.position.copy(target).add(v);dirty=true;}
 ...
 renderer.domElement.addEventListener('contextmenu',e=>e.preventDefault());renderer.domElement.addEventListener('pointerdown',e=>{if(cameraLocked)return;renderer.domElement.setPointerCapture(e.pointerId);pointers.set(e.pointerId,{x:e.clientX,y:e.clientY,button:e.button});});
 ...
 for(const event of ['pointerup','pointercancel'])renderer.domElement.addEventListener(event,e=>{pointers.delete(e.pointerId);lastPinch=0;});renderer.domElement.addEventListener('wheel',e=>{if(cameraLocked)return;e.preventDefault();zoom(Math.exp(e.deltaY*.001));},{passive:false});
 ...
 function resize(){const w=host.clientWidth,h=host.clientHeight;if(w<=0||h<=0)return;renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix();dirty=true;}
 ...
 window.BC_VIEWER.setCameraLocked=setCameraLocked;
