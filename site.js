...
window.BC_SITE=(T,h,data)=>{
  ...
  const rear=slab([[0,30],[46,30],[46,data.rearClip.y],[0,data.rearClip.y]],L.forecourt-.2,.2,concrete,ground);rear.name='REAR-WORKSHOP-CONTEXT-EXTENSION';rear.userData={notPropertyBoundary:true,status:'photo-fit working ground, not survey'};
  ...
  segments(fenceEdge,1.8,(x,y,len,a,s)=>{if(x<raw[0][0]||openGate(x,y))return;ibox(x,wallTop-.07,-y,len,.12,.25,retaining,objects,a);if(Math.floor(s/1.8)%2===0)ibox(x,wallTop-.16,-y,.24,.12,.255,black,objects,a);});
  ...
  const shrine=window.BC_SHRINE(T,{mesh},{rotation:Math.PI});shrine.name='EXISTING-CHINESE-SHRINE';shrine.position.set(data.markers.shrine.xy[0],L.forecourt,-data.markers.shrine.xy[1]);shrine.userData={...shrine.userData,...data.markers.shrine};objects.add(shrine);
  ...
  for(const x of [21.8,43.6]){const y=x>40?-9.2:-10.15;irod([x,L.sidewalk,-y],[x,7.7,-y],.061,M.steel,occluders);irod([x,7.7,-y],[x+1.8,8.15,-y+.4],.039,M.steel,occluders);ibox(x+1.85,8.13,-y+.42,.65,.09,.21,M.steel,occluders);}
  ...
  label('FORECOURT · EXISTING CONTEXT',17,-4.9,13);label('PUBLIC ROAD EDGE · TBC',23,-12,16);label('SIDE APRON · TBC',44,13,7);label('MODEL CROP · NOT PROPERTY LINE',23,data.rearClip.y-1,19);
