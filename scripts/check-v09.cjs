...
test('R9 preserves building and smart module, with only audited rounded backdrop, logo artwork and handover metadata deltas',()=>{
 assert.equal(d.revision,'v09');assert.equal(d.iteration,'r9');assert.equal(d.designRevision,'v15');
...
test('R9 Module3B canonical rounded trapezoid reaches all three states without changing floor pose or furniture',()=>{
...
test('Owner photo-fit parking has 26 cells, 10 illustrative cars, mixed orientations and unique canonical paint',()=>{
 const p=JSON.parse(read('assets/parking-photo-fit.json'));assert.deepEqual(d.site.exteriorParking,{...p,level:d.site.levels.forecourt});assert.equal(p.revision,'owner-v15-r9');assert.equal(p.siteCapacity,null);assert.equal(p.identifiedCompleteCells,26);assert.equal(p.cells.length,26);assert.equal(new Set(p.cells.map(c=>c.id)).size,26);assert(!p.cells.some(c=>c.id==='OWNER-R27-P1'));
...
test('Meeting outside Sales and attached workshop are registered separately',()=>{const a=d.site.annexes;assert.equal(a.meeting.fitEnvelope.xMax,0);assert.equal(a.workshop.footprint.yMin,16);assert.equal(a.workshop.attachment.y,16);assert.equal(a.workshop.awning.xMin,a.workshop.footprint.xMax);assert.equal(d.site.rearClip.y,50);assert.equal(d.site.edgeControlPoints.at(-1)[1],42,'r9 roadside fence covers full workshop length');assert.equal(d.exterior.serviceCanopy.replacedBy,'site.annexes.workshop.awning');});
test('R9 roadside mass, inside-only paint, two English signs and Thai left-hand traffic are explicit',()=>{
...
test('R9 actual annex factory renders one full rear roof and six posts outside all ten paint cells',()=>{
...
test('Owner selects Small two-screen Sub Stage and confirms MB6 via vacated MB5 then left down existing Entrance ramp',()=>{
 ...
 const r=d.site.entryRamp;assert.deepEqual(without(r,['status','render','mechanism','displayState','deployedOnlyForVehicleTransfer']),{xMin:26.6,xMax:29.8,yMin:-4.5,yMax:2.5});assert.match(r.status,/schematic pending measurement/);assert.equal(r.render,false);assert.equal(r.deployedOnlyForVehicleTransfer,true);assert.equal(r.mechanism,'convertible-steps-ramp');assert.equal(d.site.entrySteps.render,true);assert.equal(d.site.entrySteps.displayState,'steps');assert.equal(d.site.entrySteps.antiSlipPerStep,3);
...
test('Three native plans and three clean review maps retain exact dist parity',()=>{
 const p=JSON.parse(read('assets/presentation-v11.json'));assert.equal(p.geometryRevision,'v09-r9');assert.equal(p.presentationRevision,'v15');assert.equal(p.experienceRevision,'v11');
...
test('Exterior review-map image preserves all 104 cell vertices at historical metric pin positions',()=>{
 ...
 const manifest=JSON.parse(read('assets/presentation-v11.json')),entry=manifest.reviewMaps.find(q=>q.id==='groundfloor-site-plan');assert(entry);assert.equal(entry.path,'assets/plans/groundfloor-site-plan-review-map-v15.svg');
