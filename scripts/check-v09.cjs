...
test('R6 preserves building and smart module, with only owner-confirmed logo artwork and handover metadata deltas',()=>{
 assert.equal(d.revision,'v09');assert.equal(d.iteration,'r6');assert.equal(d.designRevision,'v12');
 ...
 // separately and assembled below; the only other deltas are wall artwork and
 ...
 assert.equal(actual,expected,'Only verified Type4, source wall-logo and same-geometry consult desk metadata allowed');
...
test('Owner photo-fit parking has 26 cells, 10 illustrative cars, mixed orientations and unique canonical paint',()=>{
 const p=JSON.parse(read('assets/parking-photo-fit.json'));assert.deepEqual(d.site.exteriorParking,{...p,level:d.site.levels.forecourt});assert.equal(p.revision,'owner-v12-r6');assert.equal(p.siteCapacity,null);assert.equal(p.identifiedCompleteCells,26);assert.equal(p.cells.length,26);assert.equal(new Set(p.cells.map(c=>c.id)).size,26);assert(!p.cells.some(c=>c.id==='OWNER-R27-P1'));
...
test('Ten rear workshop bays retain 2.5×5m dimensions, 1m wall offset, right-to-left numbering and no crossed-out bays',()=>{
 ...
 const cover=d.site.annexes.workshop.rearCanopy;assert.equal(cover.id,'EXISTING-REAR-PARKING-CANOPY');assert.equal(cover.render,true);near(cover.xMin,15);near(cover.xMax,40);near(cover.wallY,42);near(cover.outerY,48.3);near(cover.supportY,48.1);assert.deepEqual(cover.supportXs,[15,20,25,30,35,40]);assert.match(cover.status,/photo-fit|not measured/i);
});
...
test('Three native plans and three clean review maps retain exact dist parity',()=>{
 const p=JSON.parse(read('assets/presentation-v11.json'));assert.equal(p.geometryRevision,'v09-r6');assert.equal(p.presentationRevision,'v12');assert.equal(p.experienceRevision,'v11');
...
test('Exterior review-map image preserves all 104 cell vertices at historical metric pin positions',()=>{
 ...
 const manifest=JSON.parse(read('assets/presentation-v11.json')),entry=manifest.reviewMaps.find(q=>q.id==='groundfloor-site-plan');assert(entry);assert.equal(entry.path,'assets/plans/groundfloor-site-plan-review-map-v12.svg');
