...
fetch('./assets/smart-review-matrix.json?v=12-r6').then(r=>{if(!r.ok)throw Error('matrix');return r.json();}).then(data=>{
 for(const [key,label] of Object.entries(areas)){
  ...
  for(const row of data.rows.filter(r=>r.area===key)){
   ...
   for(const text of [row.requirement,row.v12Intent||row.v09Intent||row.v07Intent,row.review])if(text){const p=document.createElement('p');p.textContent=text;article.append(p);}
