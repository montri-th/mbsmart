...
window.BC_EXTERIOR_DESIGN=(T,h,data,siteData)=>{
  ...
  function board(d){const g=tag(new T.Group(),d);proposed.add(g);g.position.set(d.x,base,-d.y);box(0,d.height/2,0,d.width,d.height,d.depth,dark,g);return g;}
  const p=data.pylon,pg=board(p);planBox(0,0,p.width+.16,p.depth+.23,.08,silver,0,pg);
  ...
  const fd=data.flag;const flag=window.BC_SITE_FLAG(T,h,{...fd,kind:'smart',base});tag(flag,fd);proposed.add(flag);
